...
class EmployeeImageService:
    def __init__(
        self, repo: EmployeeImageRepo = Depends(), file_db: FileDBClient = Depends()
    ):
        self.__repo = repo
        self.__file_db = file_db

    async def create(
        self, params: params.Create, form: forms.Create
    ...
    async def update(
        ...
        if obj is None:
            raise HTTPException(
                status.HTTP_400_BAD_REQUEST, "EmployeeImage cannot be updated"
            )  # noqa

        return responses.Update(
            item=EmployeeImageScheme.model_validate(obj, from_attributes=True)
        )
    ...
    async def list(self, params: params.List) -> responses.List:  # type: ignore
        ...
        return responses.List(
            items=[
                EmployeeImageScheme.model_validate(obj, from_attributes=True)
                for obj in items
            ],
            total=total,
        )
    ...
    async def __update_obj(
        self, obj: EmployeeImageModel, body: bodies.Update
    ) -> EmployeeImageModel:
